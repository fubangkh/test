...
# --- 常量定义 ---
ACCOUNTS_LIST = [
    "ABA_924_个人户", "ABA_403_个人户", "ABA_313_FB公司户","ICBC_215_AF公司户", 
    "BOC_052_FB公司户", "BOC_063_FB公司户", "BOC_892_瑞尔_FB公司户", "ICBC_854_FB公司户", 
    "CCB_762_人民币_个人户", "BOC_865_人民币_亚堡公司户", "CCB_825_美元_昆仑公司户", 
    "CCB_825_港币_昆仑公司户", "CCB_825_人民币_昆仑公司户", "CMB_002_人民币_科吉公司户", 
    "CMB_032_美元_科吉公司户", "ABA_357_定期", "HUONE_USD", "HUONE_USDT", "现金"
]
...
if role == "数据录入" and password == STAFF_PWD:
    ...
    with col2:
        account_type = st.selectbox("结算账户", ACCOUNTS_LIST)
    ...
    if st.button("🚀 确认提交"):
        ...
        if not summary or handler in ["🔍 选择", ""]: st.error("❌ 摘要和经手人不能为空！")
            ...
            try:
                ...
                new_row = {"日期": report_date.strftime('%Y-%m-%d'), "摘要": summary, "客户/项目名称": project_name, "账户": account_type, "审批/发票编号": ref_no, "资金性质": fund_property, "收入": inc, "支出": exp, "余额": last_balance + inc - exp, "经手人": handler, "备注": auto_note}
    ...
    if not df_latest.empty:
        ...
        with st.form("edit_form"):
            ...
            with c2:
                ...
                new_acc = st.selectbox("账户", ACCOUNTS_LIST, index=ACCOUNTS_LIST.index(row_to_edit["账户"]) if row_to_edit["账户"] in ACCOUNTS_LIST else 0)
            ...
            if st.form_submit_button("💾 保存并校准全表"):
                df_latest.at[edit_id, "日期"], df_latest.at[edit_id, "摘要"], df_latest.at[edit_id, "资金性质"] = new_date.strftime('%Y-%m-%d'), new_summary, new_prop
                df_latest.at[edit_id, "收入"], df_latest.at[edit_id, "支出"], df_latest.at[edit_id, "账户"] = new_inc, new_exp, new_acc
                df_latest.at[edit_id, "客户/项目名称"], df_latest.at[edit_id, "经手人"], df_latest.at[edit_id, "备注"] = new_proj, new_hand, new_note
                cur_bal = 0.0
...
# --- 3. 管理看板 (增强：账户明细) ---
    ...
    if not df_latest.empty:
        ...
        df_before = df_vis[df_vis['日期'] < first_day]
        opening_bal = df_before.iloc[-1]['余额'] if not df_before.empty else 0.0
        df_month = df_vis[df_vis['日期'] >= first_day]
        ...
        m4.metric("💰 账户当前总结余", f"${total_bal:,.2f}")

        st.divider()
        
        # 2. 核心新增：账户汇总表 (类似截图 image_792920)
        st.subheader("🏦 各银行账户资金分布 (USD)")
        
        # 按账户分组计算
        acc_summary = []
        for acc in ACCOUNTS_LIST:
            df_acc = df_vis[df_vis['账户'] == acc]
            if not df_acc.empty:
                # 账户期初：本月1号之前的余额
                acc_before = df_acc[df_acc['日期'] < first_day]
                # 注意：账户期初需要累计计算
                acc_opening = acc_before['收入'].sum() - acc_before['支出'].sum()
                # 本月收支
                acc_month = df_acc[df_acc['日期'] >= first_day]
                acc_inc = acc_month['收入'].sum()
                acc_exp = acc_month['支出'].sum()
                acc_final = acc_opening + acc_inc - acc_exp
                
                if abs(acc_final) > 0.001 or abs(acc_inc) > 0.001 or abs(acc_exp) > 0.001:
                    acc_summary.append({
                        "账户名称": acc,
                        "期初结余": acc_opening,
                        "本月收入": acc_inc,
        ...
        if acc_summary:
            df_acc_table = pd.DataFrame(acc_summary)
            # 格式化显示
            st.table(df_acc_table.style.format({
                "期初结余": "${:,.2f}", "本月收入": "${:,.2f}", 
                "本月支出": "${:,.2f}", "当前结余": "${:,.2f}"
            }))
        else:
