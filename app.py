...
role = st.sidebar.radio("功能选择", ["数据录入", "汇总统计"]) # 移除了“数据修改”选项
        ...
        if st.form_submit_button("🚀 确认提交录入", use_container_width=True):
            if not summary or not handler:
                ...
                row = {
                    ...
                    "日期": report_date.strftime('%Y-%m-%d'), "摘要": summary, "客户/项目名称": "",
                    "账户": acc_type, "资金性质": fund_prop, 
...
# B. 汇总统计 (含嵌入式修改功能)
    ...
    if not df_latest.empty:
        ...
        # --- 重点：嵌入式修改表单 ---
        with st.expander("🛠️ 发现错误？点击此处快速修改选定流水"):
            ids = df_latest["录入编号"].astype(str).tolist()[::-1]
            col_sel, col_info = st.columns([1, 2])
            with col_sel:
                target_id = st.selectbox("请选择要修改的流水号", ["--请选择--"] + ids)
            ...
            if target_id != "--请选择--":
                ...
                with st.form("quick_edit_form"):
                    ...
                    if st.form_submit_button("💾 保存修改并刷新清单", use_container_width=True):
        ...
        # 数据表格显示
        st.divider()
        st.markdown(f"##### 📑 {selected_month} 流水明细")
